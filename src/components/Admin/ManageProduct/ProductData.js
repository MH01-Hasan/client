...
const ProductData = ({ Product, index, setlodding, setManageProduct, ManageProduct }) => {

    const { _id, Brand_Name, Catagory, Model, Product_Name, image, price } = Product;
            ...
            axios.delete(`http://localhost:5000/Product/${id}`)
                ...
                .then(data => {
                    if (data.deletedCount > 0) {
                        const remainguser = ManageProduct?.filter(manages => manages._id !== id)
    ...
    return (
        ...
        <tr>
            <td>{index + 1}</td>
            <td><img src={image} alt="" className='manageproduct-image' /></td>
            <td>{Product_Name}</td>
            <td>{Catagory}</td>
            <td>{Model}</td>
            <td>{Brand_Name}</td>
